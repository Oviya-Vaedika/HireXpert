...
def get_dynamic_suggestion(resume_text):
    ...
    for industry, keywords in industry_profiles.items():
        try:
            ...
            # Use flatten() or float conversion to handle scalar comparison safely
    ...
    if resume_text:
            ...
            if st.button("🔍 Analyze Resume", use_container_width=True):
                ...
                try:
                    ...
                    if score >= 60:
                        ...
                        st.warning(f"❌ Low Match. Your profile aligns closer to: **{industry}**")
                except:
        ...
        with col2:
            if st.button("✨ Improve Resume", use_container_width=True):
                ...
                # Extract words with 5+ letters as potential keywords
                ...
                if missing:
                    st.info(", ".join(missing[:15]))
                else:
                    st.success("No major keywords missing!")
                
                st.write("#### 💡 Quick Tips:")
                st.write("- **Bold** key skills to catch the eye.")
                st.write("- Use **bullet points** for readability.")
                st.write("- Focus on **measurable achievements** (e.g., 'Increased sales by 20%').")
    else:
